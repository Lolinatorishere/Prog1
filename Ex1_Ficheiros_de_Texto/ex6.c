...
int main(){
    ...
    //b. Nº do(s) aluno(s) com nota to TP mais alta.
    //c. Nº do(s) aluno(s) com nota mais baixa no teste1.
    int alunoBaixo[2];//n aluno, sua nota
    alunoBaixo[0] = alunos[0].aluno;
    alunoBaixo[1] = alunos[0].teste1;
    int alunoAlto[2];//n aluno, sua nota
    alunoAlto[0] = alunos[0].aluno;
    alunoAlto[1] = alunos[0].TP;
    for(int i = 0 ; i < j ; i++){
        if(alunos[i].teste1 < alunoBaixo[1]){
            alunoBaixo[1] = alunos[i].teste1;
            alunoBaixo[0] = alunos[i].aluno;
        }
        if(alunos[i].TP > alunoAlto[1]){
            alunoAlto[1] = alunos[i].TP;
            alunoAlto[0] = alunos[i].aluno;
        }
        notas[0] += alunos[i].teste1;
        notas[1] += alunos[i].teste2;
        notas[2] += alunos[i].TP;
    }
    notas[0] = notas[0]/j;
    notas[1] = notas[1]/j;
    notas[2] = notas[2]/j;
    printf("\nMedia Nota teste1: %.2f", notas[0]);
    printf("\nMedia Nota teste2: %.2f", notas[1]);
    printf("\nMedia Nota TP: %.2f", notas[2]);
    printf("\nAluno: %i tem a nota mais baixa no teste1 de:%i", alunoBaixo[0],alunoBaixo[1]);
    printf("\nAluno: %i tem a nota mais Alta no TP de:%i", alunoAlto[0],alunoAlto[1]);
